if (isServer) then
{
	...
	_name = name _unit;
		...
		_get = nil;
		while {isNil("_get")} do {
		_get = "Arma2Net.Unmanaged" callExtension format ["Arma2NETMySQLCommandAsync ['koc_koth', '%1']", _query];
		if (_get == "") then {
			_get = nil;
		};
		sleep 0.5;
		...
		if ((count (toArray(_get))) < 6) then { //if user doesnt exist in database
			...
			_query = "INSERT into player (uid, name, xp, kills, deaths, uniform, uniformitems) VALUES(";
		
			_query = _query + "'" + (_uid) + "'," + "'" + (_name) + "'" + ",0,0,0,NULL,NULL)";

			_return = nil;
			while {isNil("_return")} do {
				_return = "Arma2Net.Unmanaged" callExtension format ["Arma2NETMySQLCommandAsync ['koc_koth', '%1']", _query];
				if (_return == "") then {
					_return = nil;
				};
				sleep 0.5;
